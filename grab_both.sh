...
CAMAUDDEVICE="alsa_input.usb-046d_HD_Pro_Webcam_C920_4BB47EAF-02.analog-stereo"
if [ -n "$HEADSET" ]; then
    CAMAUDDEVICE="alsa_input.usb-Logitech_Inc_Logitech_USB_Headset_H540_00000000-00.analog-stereo"
fi
SCOPEAUDDEVICE="alsa_input.pci-0000_00_1b.0.analog-stereo"
...
gst-launch-1.0 -ve \
    v4l2src device="$SCOPEVIDDEVICE" typefind=true ! \
        ...
        tee name=vid1 \
    pulsesrc device="$SCOPEAUDDEVICE" ! \
    ...
    v4l2src device="$CAMVIDDEVICE" ! \
        ...
        tee name=vid2 \
    pulsesrc device="$CAMAUDDEVICE" ! \
        audio/x-raw,format=S16LE,rate=48000,channels=2 ! identity sync=true ! \
        queue max-size-buffers=0 max-size-bytes=0 max-size-time=0 name=audioq2 \
    vid2. ! queue ! \
